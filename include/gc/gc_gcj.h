...
/* This file assumes the collector has been compiled with           */
/* `GC_GCJ_SUPPORT` macro defined.                                  */
...
/* This function must be called before the `gcj` allocators are         */
/* invoked.  `mp_index` and `mp` are the index and mark proc (see       */
/* `gc_mark.h` file), respectively, for the allocated objects.          */
/* `mp` will be used to build the descriptor for objects allocated      */
/* through the debugging interface; it will be invoked on all such      */
/* objects with an `env` value of 1.  The client may choose to use the  */
/* same mark procedure (`mp`) for some of its generated mark            */
/* descriptors.  In that case, it should use a different `env` value to */
/* detect the presence or absence of the debug header.  `mp` is really  */
/* of type `GC_mark_proc`, as defined in `gc_mark.h` file; we do not    */
...
/* (`GC_GCJ_RESERVED_MARK_PROC_INDEX` in `gc_mark.h` file) is an        */
/* obvious choice.  Deprecated, portable clients should include         */
/* `gc_mark.h` file and use `GC_init_gcj_malloc_mp()` instead.          */
...
/* Similar to `GC_gcj_malloc`, but add the debug info.  This is         */
/* allocated with `GC_gcj_debug_kind`.                                  */
GC_API GC_ATTR_MALLOC GC_ATTR_ALLOC_SIZE(1) void *GC_CALL
