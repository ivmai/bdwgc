...
/* to be reclaimed.  If `proc()` returns a non-zero value, the          */
...
/* be protected from collection if `mark_from_all` is non-zero, but at  */
...
/* the argument) without inquiring `proc()`.  Acquires the allocator    */
/* lock.  No-op in the find-leak mode.                                  */
...
/* collection.  Note that `GC_size()` (applied to such allocated        */
/* object) returns a value slightly bigger than the specified           */
/* allocation size, and that `GC_base()` result points to               */
/* a "pointer-sized" word prior to the start of the allocated object.   */
