...
/* an issue if `GC_get_all_interior_pointers()` returns 0 or if         */
/* `GC_get_dont_add_byte_at_end()` returns 1.  This interface is most   */
/* useful for compilers that generate C code.  It is also used          */
/* internally for thread-local allocation.  A manual use is hereby      */
/* discouraged.  Clients should include `atomic_ops.h` file (or         */
/* similar) before this header.  There is no debugging variant of this  */
/* allocation API.                                                      */
...
/* of size `lg` (in granules), putting the resulting pointer in         */
/* `result`.  `tiny_fl` is a "tiny" free-list array, which will be used */
/* first, if the size is appropriate.  If `lg` argument is too large,   */
/* we allocate with `default_expr` instead.  If we need to refill the   */
/* free list, we use `GC_generic_malloc_many()` with the indicated kind */
/* `k`.  `tiny_fl` should be an array of `GC_TINY_FREELISTS` `void`     */
/* pointers.  If `num_direct` is nonzero, and the individual free-list  */
/* pointers are initialized to `(void *)1`, then we allocate            */
/* `num_direct` granules directly using `GC_generic_malloc_many()`      */
/* before putting multiple objects into the `tiny_fl` entry.            */
/* If `num_direct` is zero, then the free lists may also be initialized */
/* to `NULL`.  Note that we use the zeroth free list to hold objects 1  */
/* granule in size that are used to satisfy size 0 allocation requests. */
/* We rely on much of this hopefully getting optimized away in the case */
/* of `num_direct` is 0.  Particularly, if `lg` argument is constant,   */
