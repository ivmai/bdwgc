...
/* we return `(*GC_oom_fn)(size)`.  If it returns, it must return       */
/* either `NULL` or a valid pointer to a previously allocated heap      */
/* object.  By default, this just returns `NULL`.  If it points to      */
/* a function which never returns `NULL`, probably by aborting the      */
/* program instead, then invocations of `GC_MALLOC()` and friends (that */
/* are additionally marked as                                           */
/* "never returning NULL unless GC_oom_fn returns NULL") do not need to */
...
/* the `FINALIZE_ON_DEMAND` macro is defined when the collector         */
/* is built.  The setter and the getter are unsynchronized.             */
...
/* `GC_register_finalizer_unreachable()` to work correctly.  The setter */
/* and the getter are unsynchronized.                                   */
...
/* perfectly reasonable.  The setter and the getter are unsynchronized, */
/* so `GC_call_with_alloc_lock()` (`GC_call_with_reader_lock()` in case */
...
/* nanoseconds as given in the latest `GC_set_time_limit_tv()` call,    */
/* if any).  Not a hard time bound.  Setting this variable to           */
/* `GC_TIME_UNLIMITED` essentially disables incremental collection      */
/* (i.e. disables the "pause time exceeded" tests) while leaving        */
/* generational collection enabled.  The setter and the getter are      */
...
/* 10000 unless `GC_ALLOCD_BYTES_PER_FINALIZER` macro with a custom     */
/* value is defined to build the collector.  The default value might be */
/* not the right choice for clients where e.g. most objects have        */
/* a finalizer.  Zero value effectively disables taking amount of       */
/* finalizers in the decision whether to collect or not.  The functions */
/* do not use any synchronization.                                      */
...
/* initialization if the collector logging is enabled).  The result is  */
/* in nanoseconds.  The function acquires the allocator lock (in the    */
/* reader mode) to avoid data race.  Defined only if the library has    */
/* been compiled without `NO_CLOCK` macro defined.                      */
...
/* they are going to execute code on any of the GC-allocated memory     */
/* objects.                                                             */
...
/* `GC_set_pages_executable` (before `GC_INIT()` call) unless the       */
/* former has no effect on the platform.  Does not use or need          */
/* synchronization.                                                     */
...
/* Set/get the size in pages of units operated by                       */
/* `GC_collect_a_little()`.  The value should not be zero.              */
/* Not synchronized.                                                    */
...
/* `GC_set_handle_fork()` with non-zero argument if going to use `fork` */
/* with the GC functions called in the child process.  (Note that such  */
/* client and at-fork handler activities are not fully                  */
/* POSIX-compliant.)  `GC_set_handle_fork()` instructs `GC_init` to     */
/* setup GC fork handlers using `pthread_atfork()`, the latter might    */
/* fail (or, even, absent on some targets) causing `abort` at the       */
/* collector initialization.  Issues with missing (or failed)           */
/* `pthread_atfork()` could be avoided by invocation of                 */
/* `GC_set_handle_fork(-1)` at application start-up and surrounding     */
/* each `fork()` with the relevant                                      */
...
/* `fork` result is non-zero); `GC_atfork_child()` is to be called      */
/* immediately in the child branch (i.e., `fork` result is 0).  Note    */
/* that `GC_atfork_child()` call should, of course, precede             */
...
/* Win32 target.)  A duplicate invocation is a no-op.                   */
/* `GC_INIT()` should not be called after the shutdown.  See also       */
/* `GC_win32_free_heap()`.                                              */
...
/* not appear to be reachable.  `GC_malloc_uncollectable()` and         */
...
/* `GC_malloc_uncollectable`) are guaranteed never to return `NULL`     */
...
/* does nothing).  Should be followed typically by                      */
/* `GC_reachable_here()` called for each of the stored pointers.        */
...
/* collected heap, 0 otherwise.  Primary use is as a fast alternative   */
/* to `GC_base()` to check whether the given object is allocated by the */
/* collector or not.  It is assumed that the collector is already       */
/* initialized.                                                         */
...
/* here or with the standard C library, your code is broken.  Probably, */
/* it should not have been invented, but now we are stuck.              */
/* The resulting object has the same kind as the original.  It is an    */
/* error to have changes enabled for the original object.  It does not  */
/* change the content of the object from its beginning to the minimum   */
/* of old size and `new_size_in_bytes`; the content above in case of    */
/* object size growth is initialized to zero (not guaranteed for atomic */
/* object type).  The function follows ANSI conventions for `NULL`      */
...
/* pointer to the allocated memory.  For a non-zero                     */
/* `new_size_in_bytes`, the function (including its debug variant) is   */
...
/* Add a root segment.  Wizards only.  May merge adjacent or            */
/* overlapping segments if appropriate.  Both segment start             */
/* (`low_address`) and end (`high_address_plus_1`) are not needed to be */
/* pointer-aligned.  `low_address` must not be greater than             */
/* `high_address_plus_1`.                                               */
...
/* be called frequently, and should be reasonably fast.  (`stop_func()` */
/* is called with the allocator lock held and the world might be        */
/* stopped; it is not allowed for `stop_func()` to manipulate pointers  */
/* to the garbage-collected heap or call most of GC functions.)         */
...
/* be aborted faster than normal pause times for incremental            */
/* collection; however, aborted collections do no useful work; the next */
/* collection needs to start from the beginning.  `stop_func` must not  */
/* be 0.  `GC_try_to_collect()` returns 0 if the collection was aborted */
/* (or the collections are disabled), 1 if it succeeded.                */
typedef int(GC_CALLBACK *GC_stop_func)(void);
GC_API int GC_CALL GC_try_to_collect(GC_stop_func /* `stop_func` */)
    GC_ATTR_NONNULL(1);

/* Set and get the default `stop_func`.  The default `stop_func` is     */
/* used by `GC_gcollect()` and by implicitly triggered collections      */
/* (except for the case when handling out of memory).  Must not be 0.   */
/* Both the setter and the getter acquire the allocator lock (in the    */
/* reader mode in case of the getter) to avoid data race.               */
...
/* for any argument.  Returned (filled in) values are of `GC_word`      */
/* type.                                                                */
...
struct GC_prof_stats_s {
  ...
  /* the value of returned by `GC_get_parallel()` (or 0 if the          */
  ...
  /* Number of bytes freed explicitly since the recent garbage          */
  /* collection.  Same as the value returned by                         */
...
/* should pass the size of the buffer (of `GC_prof_stats_s` type) to    */
/* fill in the values - this is for interoperability between different  */
/* collector versions, an old client could have fewer fields, and vice  */
/* versa, client could use newer `gc.h` file (with more entries         */
/* declared in the structure) than that of the linked collector         */
/* library; in the latter case, unsupported (unknown) fields are filled */
/* in with -1 (`~0`).  Return the size (in bytes) of the filled in part */
/* of the structure (excluding all unknown fields, if any).             */
...
/* `GC_size_map` table which provides requested-to-actual allocation    */
/* size mapping.  Assumes the collector is initialized.  Returns -1     */
/* (`~0`) if the index is out of `GC_size_map` table bounds.  Does not  */
/* use synchronization, thus clients should call it using               */
...
/* Try to re-enable garbage collection.  `GC_disable()` and             */
/* `GC_enable()` calls could be nested.  Garbage collection is enabled  */
/* if the number of calls to both functions is equal.                   */
...
/* (atomic) or immutable.  Do not use in the leak finding mode.         */
/* Ignored if `GC_dont_gc` is non-zero.  Only the generational piece of */
...
/* `NULL` (for a non-zero new size in case of `GC_REALLOC`) unless      */
/* `GC_oom_fn()` returns `NULL`.                                        */
...
/* finalizer must be run when the object is known to be no longer       */
/* reachable, not even from other finalizable objects.  It behaves like */
/* "normal" finalization, except that the finalizer is not run while    */
/* the object is reachable from other objects specifying unordered      */
/* finalization.  Effectively it allows an object referenced, possibly  */
/* indirectly, from an unordered finalizable object to override the     */
/* unordered finalization request.  This can be used in combination     */
/* with `GC_register_finalizer_no_order` so as to release resources     */
/* that must not be released while an object can still be brought back  */
/* to life by other finalizers.  Only works if `GC_java_finalization`   */
...
/* finalizable objects, thus causing cyclic finalizable objects to be   */
/* finalized in the correct order.  The standard use involves calling   */
...
/* A slight generalization of the above.  `*link` is cleared when `obj` */
/* first becomes inaccessible.  This can be used to implement weak      */
/* pointers easily and safely.  Typically `link` will point to          */
/* a location (in a GC-allocated object or not) holding a disguised     */
/* pointer to `obj`.  (A pointer inside an "atomic" object is           */
/* effectively disguised.)  In this way, weak pointers are broken       */
...
/* This function can be used to implement certain types of weak         */
/* pointers.  Note, however, this generally requires that the allocator */
/* lock is held, at least in the reader mode (e.g. using                */
/* `GC_call_with_reader_lock()`), when the disguised pointer is         */
/* accessed.  Otherwise a strong pointer could be recreated between the */
/* time the collector decides to reclaim the object and the link is     */
/* cleared.  Returns `GC_SUCCESS` if registration succeeded (a new link */
/* is registered), `GC_DUPLICATE` if `link` was already registered      */
/* (with some object), `GC_NO_MEMORY` if registration failed for lack   */
/* of memory (and `GC_oom_fn` did not handle the problem),              */
...
/* (never returned if `new_link` is equal to `link`), `GC_NOT_FOUND` if */
/* no `link` is registered at the original location.                    */
...
/* gets cleared when `obj` becomes truly inaccessible.  An object       */
/* becomes truly inaccessible when it can no longer be resurrected from */
/* its finalizer (e.g. by assigning itself to a pointer traceable from  */
/* root).  This can be used to implement "long" weak pointers easily    */
/* and safely.                                                          */
...
/* Invoked by the collector for all objects registered for              */
/* "toggle-refs" processing.  Invoked with the allocator lock held but  */
...
/* with a strong ref, a weak one otherwise.  `obj` should be the        */
/* starting address of an object allocated by `GC_malloc`               */
/* (`GC_debug_malloc`) or friends.  Returns `GC_SUCCESS` if             */
/* registration succeeded (or no callback is registered yet),           */
/* `GC_NO_MEMORY` if it failed for a lack of memory reason.             */
...
/* `GC_set_warn_proc` can be used to redirect or filter warning         */
/* messages.  `p` may not be a `NULL` pointer.  `msg` is a `printf`     */
/* format string (`arg` must match the format).  Both the setter and    */
/* the getter acquire the allocator lock (in the reader mode in case    */
/* of the getter) to avoid data race.  In GC v7.1 and before: the       */
/* setter returned the value of old `warn_proc`.  In GC v8.2.x and      */
/* before: `msg` pointer type had no `const` qualifier.                 */
...
/* `GC_abort_func` is invoked on the collector fatal aborts (just       */
/* before OS-dependent `abort()` or `exit(1)` is called).  Must be      */
/* non-`NULL`.  The default one outputs `msg` to the platform `stderr`  */
/* provided `msg` is non-`NULL`.  `msg` is `NULL` if invoked before     */
/* `exit(1)` otherwise `msg` is non-`NULL` (i.e., if invoked before     */
/* `abort`).  Both the setter and the getter acquire the allocator lock */
/* (in the reader mode in case of the getter).  The setter does not     */
/* change `GC_abort_func` if the library has been compiled with         */
...
/* This might be useful in conjunction with                             */
/* `GC_register_disappearing_link`.  Note that unlike                   */
/* `GC_HIDE_POINTER`, inversion of the least significant bit of the     */
/* argument is not guaranteed.                                          */
...
/* thread which acquires the allocator lock in the exclusive mode       */
/* later.                                                               */
#ifdef GC_THREADS
...
/* provided) is called with the allocator lock held and, might be, with */
/* the world stopped.                                                   */
...
/* in an environment in which new threads are not automatically         */
/* registered with the collector.  It is also unfortunately hard to     */
/* implement well on many platforms.  Returns `GC_SUCCESS` or           */
/* `GC_UNIMPLEMENTED`.  Acquires the allocator lock on some platforms.  */
...
/* thread.  The GC thread handle (`gc_thread_handle`) is either the one */
/* returned by `GC_get_my_stackbottom()` or `NULL` (the latter          */
/* designates the current thread).  The caller should hold the          */
/* allocator lock (e.g. using `GC_call_with_reader_lock()` with         */
/* `release` argument set to 1), the reader mode should be enough       */
/* typically, at least for the collector itself (the client is          */
/* responsible to avoid data race between this and                      */
/* `GC_get_my_stackbottom` functions if the client acquires the         */
/* allocator lock in the reader mode).  Also, the function could be     */
/* used for setting `GC_stackbottom` value (the bottom of the           */
/* primordial thread) before the collector is initialized (the          */
/* allocator lock is not needed to be acquired at all in this case).    */
...
/* They indicate failure by invoking the corresponding print procedure  */
/* (`GC_same_obj_print_proc`, `GC_is_valid_displacement_print_proc` or  */
/* `GC_is_visible_print_proc`).                                         */

/* Checked pointer pre- and post- increment operations.  Note that      */
/* the second argument (`how_much`) is in units of bytes, not multiples */
/* of the object size.  This should either be invoked from a macro, or  */
/* the call should be automatically generated.                          */
...
/* (May succeed also if both `p` and `q` point to between heap          */
/* objects.)  Returns the first argument (`p`).  (The returned value    */
/* may be hard to use due to typing issues.  But if we had a suitable   */
/* preprocessor...)  We assume this is somewhat performance critical    */
/* (it should not be called by production code, of course, but it can   */
/* easily make even debugging intolerably slow).                        */
...
/* Always returns the argument (`p`).  Uninteresting with               */
...
/* a non-zero value if that region should be scanned.  Always called    */
...
/* Note: `_endthreadex()` is not currently marked as no-return in VC++  */
...
/* thread to call `GC_WinMain()` after initializing the collector.      */
