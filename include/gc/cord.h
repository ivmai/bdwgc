...
/* Cords have type `const char *`.  This is cheating quite a bit, and   */
/* not 100% portable.  But it means that nonempty character string      */
/* constants may be used as cords directly, provided the string is      */
/* never modified in place.  The empty cord is represented by, and can  */
/* be written as, `NULL`.                                               */
...
/* Return the substring (subcord really) of `x` with length at most     */
/* `n`, starting at position `i`.  (The initial character has position  */
/* zero.)                                                               */
...
/* multiple calls to `f1` may be replaced by a single call to `f2`.     */
/* The parameter `f2` is provided only to allow some optimization by    */
/* the client.  This terminates when the right end of this string is    */
/* reached, or when `f1` or `f2` return a non-zero value.  In the       */
/* latter case this function returns a non-zero value; otherwise it     */
/* returns 0.  The specified value of `i` must be less than             */
/* `CORD_len(x)`.                                                       */
...
/* caution.                                                             */
/* Things in `cord_pos.h` file should be treated as opaque, except as   */
...
/* later.  Return the position of the first character of `s` in `x`, or */
...
/* The resulting representation takes constant space, independent of    */
/* `i`.                                                                 */
...
/* read and the file pointer will be closed immediately.  The binary    */
/* mode restriction of `CORD_from_file` does not apply.                 */
...
/* Identical to `CORD_from_char_star`, but the result may share         */
/* structure with the argument and is thus not modifiable.              */
CORD_API const char *CORD_to_const_char_star(CORD);

/* Write a cord to a file, starting at the current position.            */
/* No trailing NUL characters and newlines are added.  Returns `EOF` if */
/* a write error occurs, 1 otherwise.                                   */
...
/* position `i` or later.  The value `i` must be less than              */
/* `CORD_len(x)`.                                                       */
CORD_API size_t CORD_chr(CORD /* `x` */, size_t /* `i` */, int /* `c` */);

/* A vague analog of `strrchr`.  Returns index of the last occurrence   */
/* of `char` `c` inside `x` at position `i` or earlier.  The value `i`  */
/* must be less than `CORD_len(x)`.                                     */
...
/* functions with corresponding names, but with the following additions */
/* and changes:                                                         */
/*   1. A "%r" conversion specification specifies a `CORD` argument.    */
/*      Field width, precision, etc. have the same semantics as for     */
/*      "%s".  (Note that "%c", "%C", and "%S" were already taken.)     */
...
/*      copied; the result shares the original structure.  This may     */
/*      make them very efficient in some unusual applications.  The     */
/*      format string is copied.                                        */
/*                                                                      */
