...
/* implementation, but the size is known to clients.  Also the          */
/* implementation of some exported macros relies on it.  Do not use     */
/* anything defined here and not in `cord.h` file.                      */

/* The maximum depth of a balanced cord plus one.  We do not let cords  */
/* get deeper than this maximum.                                        */
...
#  define CORD_POS_INVALID 0x55555555
  ...
  /* a function, then this may point to `function_buf` containing the   */
  /* next few characters.  Always points to a valid string containing   */
  /* the current character unless `cur_end` is zero.                    */
  const char *cur_leaf;
