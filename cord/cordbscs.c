...
/* A variant of `CORD_substr` that simply returns a function node, thus */
/* postponing its work.  The 4th argument is a function that may be     */
/* used for efficient access to the `i`-th character.                   */
...
/* A variant of `CORD_substr` that assumes `i >= 0`, `n > 0`    */
/* and `i + n < length(x)`.                                     */
static CORD
...
/* The string is the concatenation of `forest` in order of decreasing   */
/* indices.  `forest[i].len >= fib(i + 1)` is assumed.                  */
...
CORD_add_forest(ForestElement *forest, CORD x, size_t len)
{
  ...
  /* consider the two cases in which `forest[i - 1]` either is or   */
  /* is not empty.                                                  */
...
/* treated as leaves.  This potentially adds one to the depth of    */
/* the final tree.                                                  */
static void
